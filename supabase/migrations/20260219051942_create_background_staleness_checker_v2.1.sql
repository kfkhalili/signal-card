        ...
        IF reg_row.data_type = 'quote' THEN
          ...
          IF NOT exchange_is_open THEN
              IF EXTRACT(EPOCH FROM (clock_timestamp() - v_fetched_at)) > 86400 THEN
                RAISE NOTICE 'Exchange closed, but data is very stale (>24h). Proceeding to update.';
                -- Do not CONTINUE here. Let it fall through to queue the update.
              ELSE
                RAISE NOTICE 'Exchange is closed and data is recent enough (<24h). Skipping quote staleness check.';
                CONTINUE; -- Safely skip, market is closed and we have recent data
              END IF;
            END IF;

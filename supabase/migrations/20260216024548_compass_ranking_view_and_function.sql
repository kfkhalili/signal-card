...
health_categorized AS (
    SELECT 
        ...
        rt.operating_cash_flow_per_share_ttm AS ocf_ps,
        rt.quick_ratio_ttm,
        CASE 
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) <= 0 AND COALESCE(rt.operating_cash_flow_per_share_ttm, 0) > 0 THEN 1 -- Fortress
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) <= 0 AND COALESCE(rt.operating_cash_flow_per_share_ttm, 0) <= 0 THEN 2 -- Burner
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) > 0 AND COALESCE(rt.operating_cash_flow_per_share_ttm, 0) > 0 THEN 3 -- Grinder
            ELSE 4 -- Zombie
        END as health_tier

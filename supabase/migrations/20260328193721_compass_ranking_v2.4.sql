...
-- 2. CREATE THE VIEW (The "Snapshot" of ranks)
...
WITH insider_impact AS (
    SELECT 
      ...
      SUM(
        CASE 
          WHEN it.acquisition_or_disposition = 'A' 
           AND (UPPER(it.transaction_type) LIKE '%PURCHASE%' OR UPPER(it.transaction_type) = 'P' OR UPPER(it.transaction_type) LIKE 'P-%')
          THEN (it.securities_transacted * it.price)
          
          WHEN it.acquisition_or_disposition = 'D' 
           AND (UPPER(it.transaction_type) LIKE '%SALE%' OR UPPER(it.transaction_type) = 'S' OR UPPER(it.transaction_type) LIKE 'S-%')
          THEN -(it.securities_transacted * it.price)
...
health_categorized AS (
    SELECT 
        ...
        -- Use the LEAST of OCF per share and EBITDA per share (revenue_per_share_ttm * ebitda_margin_ttm)
        LEAST(
        ...
        CASE 
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) <= 0 
             AND LEAST(COALESCE(rt.operating_cash_flow_per_share_ttm, 0), COALESCE(rt.revenue_per_share_ttm * rt.ebitda_margin_ttm, rt.operating_cash_flow_per_share_ttm, 0)) > 0 THEN 1 -- Fortress
             
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) <= 0 
             AND LEAST(COALESCE(rt.operating_cash_flow_per_share_ttm, 0), COALESCE(rt.revenue_per_share_ttm * rt.ebitda_margin_ttm, rt.operating_cash_flow_per_share_ttm, 0)) <= 0 THEN 2 -- Burner
             
            WHEN (COALESCE(rt.interest_debt_per_share_ttm, 0) - COALESCE(rt.cash_per_share_ttm, 0)) > 0 
             AND LEAST(COALESCE(rt.operating_cash_flow_per_share_ttm, 0), COALESCE(rt.revenue_per_share_ttm * rt.ebitda_margin_ttm, rt.operating_cash_flow_per_share_ttm, 0)) > 0 THEN 3 -- Grinder
...
normalized_metrics AS (
    SELECT
      ...
      hc.norm_health -- Replacing the old norm_de
...
-- 3. CREATE THE FUNCTION (Updated to accept p_industries array)
DROP FUNCTION IF EXISTS public.get_weighted_leaderboard(jsonb, text);
...
BEGIN
  ...
  SELECT
    RANK() OVER (ORDER BY (
        ...
        mv.norm_profitability_yield * COALESCE((weights->>'profitability')::NUMERIC, 0.2) +
        mv.norm_buyback_yield * COALESCE((weights->>'buyback')::NUMERIC, 0.15) + 
        mv.norm_div_yield * COALESCE((weights->>'income')::NUMERIC, 0.0) +
        mv.norm_health * COALESCE((weights->>'health')::NUMERIC, 0.35)
    ) DESC) as rank,
    mv.symbol,
    (
        mv.norm_ps * COALESCE((weights->>'revenue')::NUMERIC, 0.15) +

...
ADD COLUMN IF NOT EXISTS profitability_rank BIGINT,
ADD COLUMN IF NOT EXISTS buyback_rank BIGINT,
ADD COLUMN IF NOT EXISTS peg_rank BIGINT,
ADD COLUMN IF NOT EXISTS div_yield_rank BIGINT,
ADD COLUMN IF NOT EXISTS health_rank BIGINT;
...
BEGIN
    ...
    INSERT INTO public.compass_pillar_scores (
        ...
        norm_sentiment, sentiment_rank, norm_profitability_yield, profitability_rank, norm_buyback_yield, buyback_rank,
        norm_peg, peg_rank, norm_div_yield, div_yield_rank, norm_health, health_rank, updated_at
    )
    ...
    normalized_metrics AS (
        SELECT
          ...
          RANK() OVER (ORDER BY CASE WHEN p.price > 0 THEN LEAST(rt.net_income_per_share_ttm, rt.free_cash_flow_per_share_ttm) / p.price ELSE NULL END DESC NULLS LAST) as profitability_rank,
          
          -- Buyback (lower share change is better buyback yield)
          PERCENT_RANK() OVER (ORDER BY COALESCE(abs.avg_share_change, 0) DESC) * 100 AS norm_buyback_yield,
          RANK() OVER (ORDER BY COALESCE(abs.avg_share_change, 0) ASC) as buyback_rank,
          
          -- PEG (lower is better)
          (1 - PERCENT_RANK() OVER (ORDER BY rt.price_to_earnings_growth_ratio_ttm ASC)) * 100 AS norm_peg,
          RANK() OVER (ORDER BY rt.price_to_earnings_growth_ratio_ttm ASC NULLS LAST) as peg_rank,
          
          -- Div Yield
          PERCENT_RANK() OVER (ORDER BY rt.dividend_yield_ttm ASC) * 100 AS norm_div_yield,
          RANK() OVER (ORDER BY rt.dividend_yield_ttm DESC NULLS LAST) as div_yield_rank,
          
          -- Health
          hc.norm_health,
          RANK() OVER (ORDER BY hc.norm_health DESC NULLS LAST) as health_rank,
          NOW()
    ...
    ON CONFLICT (symbol) DO UPDATE SET
        ...
        buyback_rank = EXCLUDED.buyback_rank,
        norm_peg = EXCLUDED.norm_peg,
        peg_rank = EXCLUDED.peg_rank,
        norm_div_yield = EXCLUDED.norm_div_yield,
        div_yield_rank = EXCLUDED.div_yield_rank,
...
-- 3. Recreate the leaderboard RPC to point to the physical table

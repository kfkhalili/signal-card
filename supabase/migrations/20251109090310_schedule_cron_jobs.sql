...
BEGIN
  IF NOT EXISTS (
    ...
    PERFORM cron.schedule(
      ...
      SELECT
        net.http_post(
            url := current_setting('supabase.functions.url') || '/fetch-fmp-financial-statements',
            headers := jsonb_build_object(
              'Content-Type', 'application/json',
...
BEGIN
  IF NOT EXISTS (
    ...
    PERFORM cron.schedule(
      ...
      SELECT
        net.http_post(
            url := current_setting('supabase.functions.url') || '/fetch-fmp-profiles',
            headers := jsonb_build_object(
              'Content-Type', 'application/json',
              'Authorization', 'Bearer ' || (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'supabase_service_role_key')
            ),
...
BEGIN
  IF NOT EXISTS (
    ...
    PERFORM cron.schedule(
      ...
      SELECT
        net.http_post(
            url := current_setting('supabase.functions.url') || '/fetch-fmp-ratios-ttm',
            headers := jsonb_build_object(
              'Content-Type', 'application/json',
              'Authorization', 'Bearer ' || (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'supabase_service_role_key')
            ),

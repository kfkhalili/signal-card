-- Add exchange status check to background staleness checker
-- CRITICAL: Prevents creating queue jobs when exchanges are closed
-- This eliminates unnecessary jobs that will just be skipped by the processor
-- Matches the behavior of check_and_queue_stale_batch_v2 (user-facing function)
...
COMMENT ON FUNCTION public.check_and_queue_stale_data_from_presence_v2 IS 'Background staleness checker. Runs every minute. Uses Symbol-by-Symbol pattern for scalability. Quota-aware. Timeout-protected to complete within 50 seconds. Checks exchange status for quote data type to prevent creating jobs when exchanges are closed.';

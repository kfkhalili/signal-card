...
const AAPL_INITIAL_PRICE = 170.0;
const PRICE_CHANGE_MAX = 0.5; // Max change per interval
...
export function useMockPriceFeed(options?: MockPriceFeedOptions) {
  ...
  const [latestPriceData, setLatestPriceData] = useState<PriceData | null>(
    null
  );
  const [priceHistory, setPriceHistory] = useState<PriceData[]>([]);
  const [nextUpdateInSeconds, setNextUpdateInSeconds] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const priceUpdateTimer = setInterval(() => {
      setLatestPriceData((prevData) => {
        const newPriceValue = generateNewPrice(
          prevData ? prevData.price : AAPL_INITIAL_PRICE
        );

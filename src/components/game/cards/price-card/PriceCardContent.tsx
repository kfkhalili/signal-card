...
export const PriceCardContent = React.memo<PriceCardContentProps>(
  ({
    ...
    if (isBackFace) {
      ...
      // Only render price display if price is available
      const PriceDisplayBlock =
        liveData.price != null ? (
          <ClickableDataItem
            ...
            {liveData.dayChange != null || liveData.changePercentage != null ? (
              <div
                className={cn(
                ...
                {liveData.dayChange != null && (
                  <p className="text-base font-semibold" title="Day Change">
                    {`${liveData.dayChange >= 0 ? "+" : ""}${liveData.dayChange.toFixed(2)}`}
                  </p>
                )}
                {liveData.changePercentage != null && (
                  <p className="text-base font-semibold" title="Percent Change">
                    (
                    {`${liveData.changePercentage >= 0 ? "+" : ""}${liveData.changePercentage.toFixed(2)}%`}
                    )
                  </p>
                )}
              </div>

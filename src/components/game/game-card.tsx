...
const GameCard: React.FC<GameCardProps> = ({
  ...
  useEffect(() => {
    if (priceGameCard && !priceGameCard.isSecured && priceGameCard.initialFadeDurationMs && onFadedOut) {
  ...
  const handleCardClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    console.log(`GameCard (${card.id} - ${card.type}): General click. Calling onToggleFlip.`);
    onToggleFlip(card.id); 
  };
  
  const handleSmaClickForCardFace = (smaPeriod: 50 | 200, smaValue: number, receivedFaceData: PriceCardFaceData) => {
    if (isPriceGameCard && onGeneratePriceVsSmaSignal) onGeneratePriceVsSmaSignal(receivedFaceData, smaPeriod, smaValue);
  };
  const handleRangeContextClickForCardFace = (levelType: 'High' | 'Low', levelValue: number, receivedFaceData: PriceCardFaceData) => {
    if (isPriceGameCard && onGeneratePriceRangeContextSignal) onGeneratePriceRangeContextSignal(receivedFaceData, levelType, levelValue);
  };
  const handleOpenPriceClickForCardFace = (receivedFaceData: PriceCardFaceData) => {
    if (isPriceGameCard && onGenerateIntradayTrendSignal) onGenerateIntradayTrendSignal(receivedFaceData);
  };
  const handleDailyPerformanceClickForCardFace = (receivedFaceData: PriceCardFaceData) => {
    if (isPriceGameCard && onGenerateDailyPerformanceSignal) onGenerateDailyPerformanceSignal(receivedFaceData);
  };
  ...
  const frontFace = <CardFace card={card} isBack={false} onRangeContextClick={isPriceGameCard ? handleRangeContextClickForCardFace : undefined} onGenerateDailyPerformanceSignal={isPriceGameCard ? handleDailyPerformanceClickForCardFace : undefined} />;
  const backFace = <CardFace card={card} isBack={true} onSmaClick={isPriceGameCard ? handleSmaClickForCardFace : undefined} onOpenPriceClick={isPriceGameCard ? handleOpenPriceClickForCardFace : undefined} />;

  console.log(`GameCard ${card.id} (type: ${card.type}) rendering. isFlipped prop: ${card.isFlipped}`);
  ...
  return (
    ...
    >
      ...
      >
        <>
          {priceGameCard && !priceGameCard.isSecured && priceGameCard.initialFadeDurationMs && remainingTimeFormatted && (
          ...
          {isPriceGameCard && onTakeSnapshot && (
            <Button
              ...
              onClick={handleSnapshotButtonClick} data-interactive-child="true" title="Take Snapshot"
              aria-label="Take Snapshot of this card" data-testid="snapshot-button"
